...
from extensions.ebsynth_utility.stage1 import (
    ebsynth_utility_stage1,
    ebsynth_utility_stage1_invert,
)
...
def ebsynth_utility_process(
    stage_index: int,
    project_dir: str,
    original_movie_path: str,
    frame_width: int,
    frame_height: int,
    st1_masking_method_index: int,
    st1_mask_threshold: float,
    tb_use_fast_mode: bool,
    tb_use_jit: bool,
    clipseg_mask_prompt: str,
    clipseg_exclude_prompt: str,
    clipseg_mask_threshold: int,
    clipseg_mask_blur_size: int,
    clipseg_mask_blur_size2: int,
    key_min_gap: int,
    key_max_gap: int,
    key_th: float,
    ...
    st3_5_use_mask_org: bool,
    color_matcher_ref_type: int,
    color_matcher_ref_image: Image,
    blend_rate: float,
    export_type: str,
    bg_src: str,
    bg_type: str,
    ...
    frame_path = project_dir / "video_frame"
    frame_mask_path = project_dir / "video_mask"

    # if is_invert_mask:
    back_path = project_dir / "inv"
    back_mask_path = back_path / "video_mask"
    back_path.mkdir(exist_ok=True)

    back_org_key_path = back_path / "video_key"
    back_img2img_key_path = back_path / "img2img_key"
    back_img2img_upscale_key_path = back_path / "img2img_upscale_key"
    # else:
    org_key_path = project_dir / "video_key"
    img2img_key_path = project_dir / "img2img_key"
    img2img_upscale_key_path = project_dir / "img2img_upscale_key"
    ...
    if stage_index == 7:  # satage 6
        ...
        dbg.print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ebsynth_utility_stage6(
            dbg, project_dir, frame_path, img2img_key_path, frame_mask_path
        )
        ebsynth_utility_stage6(
            dbg, back_path, frame_path, back_img2img_key_path, back_mask_path
        )
        return process_end(dbg, "")
    # if stage_index == 8:  # stage 6.9
    # blending horizontally:
    # ebsynth_utility_stage6_9()
    if stage_index == 8:  # stage 7
        ebsynth_utility_stage7(dbg, project_dir, blend_rate)
        ebsynth_utility_stage7(dbg, back_path, blend_rate)

    if stage_index == 9:
        ebsynth_utility_stage7_5(
            dbg,
            project_dir,
            original_movie_path,
            frame_mask_path,
