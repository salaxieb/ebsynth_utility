...
class Script(scripts.Script):
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Accordion("Mask option"):
                mask_mode = gr.Dropdown(
                    choices=["Normal", "Invert", "None", "Don't Override"],
                    value="Normal",
                    label="Mask Mode(Override img2img Mask mode)",
                )
                inpaint_area = gr.Dropdown(
                    choices=["Whole picture", "Only masked", "Don't Override"],
                    type="index",
                    value="Only masked",
                    label="Inpaint Area(Override img2img Inpaint area)",
                )
            ...
            with gr.Accordion("ControlNet option"):
                ...
                use_preprocess_img = gr.Checkbox(
                    True,
                    label="Use Preprocess image If exists in /controlnet_preprocess",
                )
                gr.HTML(
                    value="<p style='margin-bottom: 0.7em'>\
            ...
            with gr.Accordion("Loopback option"):
                ...
                inc_seed = gr.Slider(
                    ...
                    step=1,
                    value=1,
                    label="Add N to seed when repeating ",
                )

            with gr.Accordion("Auto Tagging option"):
                auto_tag_mode = gr.Dropdown(
                    choices=["None", "DeepDanbooru", "CLIP"],
                    value="None",
                    label="Auto Tagging",
                )
    ...
    def get_mask(self):
        def create_mask(output, x_rate, y_rate, k_size):
            ...
            img = cv2.ellipse(
                img,
                ((256, 256), (int(512 * x_rate), int(512 * y_rate)), 0),
                (255, 255, 255),
                thickness=-1,
            )
    ...
    def face_img_crop(self, img, face_coords, face_area_magnification):
        ...
        for face_img in face_imgs:
            if face_img.shape[1] < face_img.shape[0]:
                ...
                re_h = int(
                    x_ceiling(
                        (self.face_crop_resolution / face_img.shape[1])
                        * face_img.shape[0],
                        64,
                    )
                )
                ...
                re_w = int(
                    x_ceiling(
                        (self.face_crop_resolution / face_img.shape[0])
                        * face_img.shape[1],
                        64,
                    )
                )
    ...
    def face_crop_img2img(
        ...
        for face, coord, controlnet_input_face in zip(
            face_imgs, new_coords, controlnet_input_face_imgs
        ):
            ...
            face_proc = self.process_images(
                face_p,
                controlnet_input_face,
                self.controlnet_weight_for_face,
                preprocess_img_exist,
            )
    ...
    def get_depth_map(self, mask, depth_path, img_basename, is_invert_mask):
        ...
        if depth_img_path.is_file():
            ...
            # try 00001-0000.png
            depth_img_path = depth_path / (Path(img_basename).stem + "-0000.png")
            if depth_img_path.is_file():
                depth = Image.open(str(depth_img_path))
    ...
    def load_prompts_dict(self, imgs, default_token):
        ...
        if not prompts_path.is_file():
            print(str(prompts_path) + " not found.")
            return {}
    ...
    def process_images(
        self, p, input_img, controlnet_weight, input_img_is_preprocessed
    ):
    ...
    def run(
        ...
        if not org_key_path.is_dir():
            ...
            print(
                ...
                if is_invert_mask == False
                else "Generate key frames first.(with [Ebsynth Utility] Tab -> [configuration] -> [etc]-> [Mask Mode] = Invert setting)"
            )
            return Processed()
        ...
        def get_pair_of_img(img, target_dir):
            ...
            if pair_path.is_file():
                return pair_path
            print("!!! pair of " + str(img) + " not in " + str(target_dir))
            return ""
        ...
        with open(project_dir / "faces.txt", "w") as f:
        ...
        with open(
            project_dir / (time.strftime("%Y%m%d-%H%M%S_") + "prompts.txt"), "w"
        ) as f:
        ...
        for img in tqdm(imgs):
            ...
            if mask_mode != "None" or use_depth:
                if use_depth:
                    depth_found, _p.image_mask = self.get_depth_map(
                        mask_image, depth_path, img.name, is_invert_mask
                    )
            ...
            controlnet_input_base_img = (
                Image.open(controlnet_input_img) if controlnet_input_img else None
            )
        ...
        with open(project_dir / "args.txt", "w") as f:
